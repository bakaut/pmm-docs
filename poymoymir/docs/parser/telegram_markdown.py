...
This module provides utilities to convert **standard Markdown** into
an unformatted string together with a list of Telegram ``MessageEntity``
objects.  The goal is to take a valid Markdown document (for example,
text generated by a large‑language model) and prepare it for sending
through the Telegram Bot API with **100 % confidence** that it will be
rendered correctly.  Instead of sending Markdown markup and relying on
Telegram’s parser to interpret it (which is notoriously fragile and
requires escaping many characters), this module strips the markup from
the message and supplies an explicit ``entities`` array describing
where each piece of formatting begins and how long it extends.  This
approach mirrors the strategy recommended by the Telegram API itself:
a client “would simply have to integrate a Markdown/HTML parser, and
generate an array of message entities by iterating through the parsed
tags”【16645222028428†L21-L26】.  Unlike parse‑mode strings, entities are
immune to special characters and do not require any escaping.

Only a subset of the most commonly used Markdown features is
supported.  These include bold (``**text**``), underline
(``__text__``), italic (``*text*`` or ``_text_``), strikethrough
(``~~text~~``), spoilers (``||text||``), inline code (``\`code\``),
preformatted code blocks using triple backticks (`````lang\ncode
``` ``) and inline links (``[label](url)``).  Nested formatting is
fully supported, and offsets are computed in terms of UTF‑16 code
units, as required by the Telegram API【16645222028428†L31-L80】.  In
addition to inline formatting, this module recognises lines
starting with ``#`` as headings and wraps them in a bold entity,
handles block quotes (``>``) and **expandable block quotes** (``>||``)
by emitting a single ``blockquote`` entity with an optional
``collapsed`` flag【885819747867534†L114-L123】, and implements simple
unordered and ordered lists using Unicode bullets (``•``, ``◦``,
``▪``) and non‑breaking spaces for indentation.  Tables and other
constructs are still passed through unchanged as plain text.

The module exposes a single public function,
``parse_markdown_to_entities()``, which accepts a Markdown string and
returns a tuple ``(message, entities)``.  The ``message`` is the
cleaned string with all formatting delimiters removed, and
``entities`` is a list of dictionaries that can be passed directly to
the Bot API under the ``entities`` parameter when sending a message.
Each entity dictionary contains a ``type`` field (for example
``"bold"`` or ``"pre"``), an ``offset`` (measured in UTF‑16 code
units), a ``length`` (also in UTF‑16 code units) and, where
applicable, additional fields such as ``url`` for links or
``language`` for code blocks【660510022329427†L125-L149】.

Example:

>>> from telegram_markdown import parse_markdown_to_entities
>>> md = "**Hello**, [world](https://example.com)!"
>>> message, entities = parse_markdown_to_entities(md)
>>> message
'Hello, world!'
>>> entities
...
The resulting ``message`` and ``entities`` may be supplied to
``sendMessage`` without specifying ``parse_mode``.  Since all
formatting is described explicitly by the entities, there is no
possibility of parse errors or of content being rendered incorrectly.

Note: This implementation is intentionally conservative.  If no
closing delimiter is found for a potential markup sequence (for
example an unmatched ``*``), the characters are treated as literal
text.  This behaviour avoids dropping characters from the output but
still honours properly paired markup when present.
