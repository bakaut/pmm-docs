...
locals {
  user_hash_rag = data.archive_file.rag_zip.output_md5
}

# Archive the function code once
data "archive_file" "rag_zip" {
  type        = "zip"
  source_dir  = "${path.module}/rag"
  output_path = "${path.module}/rag.zip"
}

resource "yandex_function" "function_rag" {

  name               = "rag"
  description        = "Cloud function RAG example"
  user_hash          = local.user_hash_rag
  ...
  environment = {
    ...
    bot_token            = data.sops_file.common_secrets.data.rag_bot_token
