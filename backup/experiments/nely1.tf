...
locals {
  user_hash_nely1 = data.archive_file.nely1_zip.output_md5
}

# Archive the function code once
data "archive_file" "nely1_zip" {
  type        = "zip"
  source_dir  = "${path.module}/nely1"
  output_path = "${path.module}/nely1.zip"
}

resource "yandex_function" "function_nely1" {

  name               = "nely1"
  description        = "Cloud function ЯГений MentorGeniyBot"
  user_hash          = local.user_hash_nely1
  ...
  environment = {
    ...
    bot_token            = data.sops_file.common_secrets.data.nely1_bot_token
