...
locals {
  user_hash_nely5 = data.archive_file.nely5_zip.output_md5
}

# Archive the function code once
data "archive_file" "nely5_zip" {
  type        = "zip"
  source_dir  = "${path.module}/nely5"
  output_path = "${path.module}/nely5.zip"
}

resource "yandex_function" "function_nely5" {

  name               = "nely5"
  description        = "Cloud function НоваяПолезнаяПривычка TrekerRostaBot"
  user_hash          = local.user_hash_nely5
  ...
  environment = {
    ...
    bot_token            = data.sops_file.common_secrets.data.nely5_bot_token
