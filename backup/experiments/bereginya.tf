...
locals {
  user_hash_bereginya = data.archive_file.bereginya_zip.output_md5
}

# Archive the function code once
data "archive_file" "bereginya_zip" {
  type        = "zip"
  source_dir  = "${path.module}/bereginya"
  output_path = "${path.module}/bereginya.zip"
}

resource "yandex_function" "function_bereginya" {

  name               = "bereginya"
  description        = "Cloud function Берегиня @SvetiLaskovoBot"
  user_hash          = local.user_hash_bereginya
  ...
  environment = {
    ...
    bot_token            = data.sops_file.common_secrets.data.bereginya_bot_token
    ...
    operouter_key        = data.sops_file.common_secrets.data.bereginya_operouter_key
  }
  content {
    zip_filename = "bereginya.zip"
  }

  log_options {
    log_group_id = yandex_logging_group.function_logs_bereginya.id
