...
class DatabaseConfig(BaseModel):
    ...
    type: str = Field(default="postgresql", description="Database type: postgresql or duckdb")
    url: Optional[str] = Field(None, description="Database connection URL (PostgreSQL)")
    path: Optional[str] = Field(None, description="Database file path (DuckDB)")
    ...
    def validate_database_type(cls, v):
        if v not in ['postgresql', 'duckdb']:
            raise ValueError('Database type must be either "postgresql" or "duckdb"')
        return v
    ...
    def validate_config_consistency(self):
        if self.type == 'postgresql':
            ...
            if not self.path:
                # Default to in-memory for DuckDB if no path specified
                self.path = ':memory:'
        return self
    ...
    def connection_params(self) -> Dict[str, Any]:
        ...
        if self.type == 'postgresql':
            ...
            return {"database": self.path or ":memory:"}
        else:
...
class PromptsConfig(BaseModel):
    ...
    system_prompt_path: str = Field(default="knowledge_bases/system_prompt.txt", description="Path to main system prompt")
    prepare_suno_path: str = Field(default="knowledge_bases/prepare_suno.txt", description="Path to Suno preparation prompt")
    intent_detection_path: str = Field(default="knowledge_bases/determinate_intent.txt", description="Path to intent detection prompt")
    emotion_detection_path: str = Field(default="knowledge_bases/detect_emotional_state.txt", description="Path to emotion detection prompt")
    state_detection_path: str = Field(default="knowledge_bases/determinate_state.txt", description="Path to state detection prompt")
    state_detection_path_v2: str = Field(default="knowledge_bases/determinate_intent_v2.txt", description="Path to state detection prompt v2")
    state_detection_path_v3: str = Field(default="knowledge_bases/determinate_state_v3.txt", description="Path to state detection prompt v3")
    ...
    _state_detection_prompt_v2: Optional[str] = None
    _state_detection_prompt_v3: Optional[str] = None
    ...
    def system_prompt(self) -> str:
        ...
        if self._system_prompt is None:
            self._system_prompt = self._load_file(self.system_prompt_path)
        return self._system_prompt

    @property
    def prepare_suno_prompt(self) -> str:
        """Load and cache Suno preparation prompt"""
        if self._prepare_suno_prompt is None:
            self._prepare_suno_prompt = self._load_file(self.prepare_suno_path)
        return self._prepare_suno_prompt

    @property
    def intent_detection_prompt(self) -> str:
        """Load and cache intent detection prompt"""
        if self._intent_detection_prompt is None:
            self._intent_detection_prompt = self._load_file(self.intent_detection_path)
        return self._intent_detection_prompt

    @property
    def emotion_detection_prompt(self) -> str:
        """Load and cache emotion detection prompt"""
        if self._emotion_detection_prompt is None:
            self._emotion_detection_prompt = self._load_file(self.emotion_detection_path)
        return self._emotion_detection_prompt

    @property
    def state_detection_prompt(self) -> str:
        """Load and cache state detection prompt"""
        if self._state_detection_prompt is None:
            self._state_detection_prompt = self._load_file(self.state_detection_path)
        return self._state_detection_prompt

    @property
    def state_detection_prompt_v2(self) -> str:
        """Load and cache state detection prompt v2"""
        if self._state_detection_prompt_v2 is None:
            self._state_detection_prompt_v2 = self._load_file(self.state_detection_path_v2)
        return self._state_detection_prompt_v2

    @property
    def state_detection_prompt_v3(self) -> str:
        """Load and cache state detection prompt v3"""
        if self._state_detection_prompt_v3 is None:
            self._state_detection_prompt_v3 = self._load_file(self.state_detection_path_v3)
        return self._state_detection_prompt_v3
...
class Config(BaseSettings):
    ...
    def validate_database_connection(self):
        ...
        if db_config.type == 'postgresql' and not db_config.url:
            # Если нет database_url для PostgreSQL, проверяем наличие fallback параметров
            required_fields = ['host', 'name', 'user', 'password']
            missing_fields = [f for f in required_fields if not getattr(db_config, f, None)]
            if missing_fields:
                raise ValueError(f'PostgreSQL requires either database_url or all fallback fields. Missing: {missing_fields}')
        return self
    ...
    @property
    def system_prompt_detect_state_v2(self) -> str:
        return self.prompts.state_detection_prompt_v2

    @property
    def system_prompt_detect_state_v3(self) -> str:
        return self.prompts.state_detection_prompt_v3
    ...
    def from_env(cls, env: Optional[Dict[str, str]] = None) -> "Config":
        ...
        try:
            ...
            config = cls(
                ...
                prompts=PromptsConfig(
                    system_prompt_path=get_env("system_prompt_path", "knowledge_bases/system_prompt.txt"),
                    prepare_suno_path=get_env("prepare_suno_path", "knowledge_bases/prepare_suno.txt"),
                    intent_detection_path=get_env("intent_detection_path", "knowledge_bases/determinate_intent.txt"),
                    emotion_detection_path=get_env("emotion_detection_path", "knowledge_bases/detect_emotional_state.txt"),
                    state_detection_path=get_env("state_detection_path", "knowledge_bases/determinate_state.txt"),
                    state_detection_path_v2=get_env("state_detection_path_v2", "knowledge_bases/determinate_intent_v2.txt"),
                    state_detection_path_v3=get_env("state_detection_path_v3", "knowledge_bases/determinate_state_v3.txt")
                )
            )
