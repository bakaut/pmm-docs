...
def handler(event: Dict[str, Any], context):  # noqa: C901
    ...
    def llm_call(messages: list[dict], max_attempts: int = 3) -> str:
        logger.debug("Preparing OpenRouter provider variants")
        provider_variants = [
            ["OpenAI", "Azure"],
            ["Azure", "OpenAI"],
            [],                           # полная автоматика OpenRouter
        ]

        for i, order in enumerate(provider_variants[:max_attempts]):
            try:
                logger.debug("Starting LLM call attempt %s", i + 1)
                logger.debug("LLM endpoint: %s", ai_endpoint)
                resp = session.post(
                    ai_endpoint,
                    json={
                        "model": ai_model,
                        "messages": messages,
                        **({"provider": {"order": order}} if order else {})
                    },
                    headers={"Authorization": f"Bearer {operouter_key}",
                            "Content-Type": "application/json",
                            "Accept": "application/json"},
                    timeout=timeout,
                    proxies=(None if i < 2 else PROXY),
                )
                ...
                # ошибка не про гео-бан → нет смысла перебирать
                if "error" in data:
                    logger.warning("LLM provider error: %s", data["error"])
                    if data["error"]["metadata"]["raw"]["error"].get("code") == "unsupported_country_region_territory":
                        pass
                    else:
                        break
            except Exception as e:
        ...
        # 🔸 все попытки исчерпаны — отдаём мягкий ответ
        logger.error("All LLM attempts failed, returning fallback answer")
